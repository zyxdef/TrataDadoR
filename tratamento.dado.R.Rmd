...
1. Organizando do Conjunto de Dados
...
1. Excel
2. Google Forms
3. Coleta biométrica de dados
(e.g., sensores, actímetro, *eye tracking* etc.)
4. Dados armazenados em arquivos de outros programas estatísticos 
(SPSS, Stata, Minitab, SAS etc.)
5. Dados oriundos de sistemas de gerenciamento de bancos de dados (SGBD), 
extraídos via, e.g., SQL
6. Dados não estruturados coletados eletronicamente
...
2. Uso inconsistente dos nomes de categorias
2. Inconsistências
3. Listagem de valores a serem verificados
...
data.frame(o.que.disse = comentário, 
    referência.verbo.gostar = grepl("(^|[ ])gost[^ ]+", comentário))
```
...
Por isso, antes de começar a mexer nos dados, 
**crie uma cópia do arquivo de dados original e deixe a salvo fora da pasta de trabalho**.
